...
## 1. 실행 전 한 줄 — **가상환경 활성화는 선택이 아닙니다**

```powershell
cd D:\battery_archive; .\.venv-blife\Scripts\Activate.ps1
```

**이 한 줄을 빠뜨리면 학습이 시작되지 않습니다.** 스크립트가 `accelerate`
를 부르는데 시스템 파이썬에는 없습니다.

```
accelerate: command not found
```

프롬프트 앞에 `(.venv-blife)` 가 붙었는지 보십시오. 새 터미널을 열 때마다
다시 활성화해야 합니다 — `run_domain.sh` 도, `Start-Process` 로 띄운
백그라운드 창도 마찬가지입니다. §3-2 의 `run_domain.sh` 는 첫 줄에서 이것을
확인하고 안 켜져 있으면 **한 줄도 돌리지 않고 멈춥니다.**
...
### 1-1. torch 를 재설치하지 마십시오 — 설치 순서가 중요합니다

`envs/requirements.txt` 를 **그대로 설치하면 torch 가 2.4.1(비 cu128)로
되돌아갑니다.** `accelerate` 의 `torch>=1.10.0` 을 pip 이 만족시키면서
cu128 휠을 일반 휠로 갈아끼우기 때문입니다. 2.4.1 은 sm_120(Blackwell)을
지원하지 않아 5080 에서 못 씁니다.

순서는 이렇습니다.

```
1. torch 를 cu128 채널에서 먼저 설치
2. requirements 설치 시 torch · deepspeed · numpy · pandas · scipy · scikit 제외
3. 설치 후 반드시 재확인            <- 이 단계를 건너뛰지 마십시오
4. 되돌아갔으면 --force-reinstall 로 복구
```

**설치 뒤 재확인은 필수입니다.** 위 확인 명령이 `2.11.0+cu128` 이 아닌
것을 뱉으면 그 환경으로는 돌리지 마십시오.

무언가를 더 설치해야 하면 **반드시 제약 파일을 걸십시오.**

```powershell
pip install -c envs\constraints.txt <패키지>
```

전체 순서와 주의는 `manifests/env_lock/blife.txt` 머리에 있습니다.
...
| 순서 | 도메인 | 셀 (train/val/test) | 샘플 | 로딩 실측 | 9개 추정 | **9개 실측 (2026-08-04)** |
|---|---|---|---|---|---|---|
| 1 | **CALB** | 17 / 5 / 5 | 2,683 | 5.1초 | 약 0.3시간 | **7분 13초** |
| 2 | **Na-ion** | 20 / 6 / 5 | 3,100 | 9.6초 | 약 0.3시간 | **6분 29초** |
| 3 | **Zn-ion** | 60 / 20 / 20 | 9,900 | 36.2초 | 약 1.1시간 | **16분 54초** |
| 4 | **Li-ion** | 510 / 165 / 162 | 82,300 | **620.4초** | 약 9.3시간 | **3시간 17분** |

**앞의 세 도메인 27회를 합쳐 30분 36초입니다** (15:29:40 → 16:00:16).
Li-ion 9회는 17:22:14 → 20:39:27 로 **3시간 17분**이고, 이 중 회당 약 10분의
로딩이 9번 들어 있어 학습 자체는 2시간 안팎입니다.

**추정(36회 약 33시간)이 크게 빗나갔습니다.** 조기 종료가 100에폭 상한보다
훨씬 이르게 걸리기 때문입니다. 근거와 대조는 `docs/PLAN.md` §2 · §3-5.

출처는 `runs/2026-08-04/*_summary.txt` 세 개와 `run_liion.log` 의 torch 경고
타임스탬프입니다. **Li-ion 은 `run_domain.sh` 로 돌리지 않아 요약 파일이
없습니다** — 시작·끝 시각이 요약으로 남지 않은 유일한 도메인입니다.
...
bash 는 **`C:\Program Files\Git\bin\bash.exe`** 입니다.

> **`usr\bin\bash.exe` 가 아닙니다.** 그쪽으로 부르면 PATH 가 서지 않아
> `ls` 조차 `command not found` 로 실패합니다. 두 파일 다 존재하기 때문에
> 경로를 잘못 적어도 bash 는 뜹니다 — 그 다음에 무너집니다.
> §4 의 `Start-Process` 예시도 `bin\bash.exe` 를 씁니다.
...
| 3에폭 실측 | `runs\2026-08-04\measure36.json` |
| 지표 | **파일로 저장되지 않습니다.** 로그의 `Best model performance:` 줄 |

### 6-1. 2026-08-04 세션의 로그는 `runs\2026-08-04\` 에 있습니다

36회를 만든 로그는 2026-08-05 에 날짜 폴더로 묶었습니다. **내용은 손대지
않았고 위치만 바뀌었습니다.** 어느 파일이 표의 어느 행을 만들었는지는
`runs\2026-08-04\README.md` 에 있습니다. 두 가지만 여기 옮겨 둡니다.

- **Zn-ion 9회는 `zn_lr9.log` 입니다.** `20260804-154322_*` 쪽은 문서
  학습률로 돌려 학습이 되지 않은 실행입니다 (§8-10).
- **Li-ion 로그는 UTF-16LE 입니다.** PowerShell 리다이렉션으로 만들어져
  `grep` 이 0건을 냅니다 — 로그가 빈 것이 아닙니다. `Select-String` 을
  쓰거나 `decode("utf-16-le")` 로 읽으십시오.

`train\collect.py --all` 은 `runs\` **바로 아래의 디렉터리**만 봅니다
(`log.txt` 를 찾습니다). 위 로그들은 낱개 `.log` 파일이라 `--all` 대상이
아닙니다 — 경로를 직접 주십시오.
...
| **Li-ion** | 코드 분할 843셀 / **논문 Table 2 는 837** | **837셀** (라벨 미배포 6셀 제외) | §8-5 |
| **Zn-ion 학습률** | 문서 CPMLP 5e-4 · CPTransformer 1e-3 | **5e-05 로 낮춤** — 문서 값으로는 학습이 안 됨 | §8-10 (TRN-012) |
| **Zn-ion 모집단** | 논문 Table 2 는 **95셀** | **코드 분할 100셀** | §8-10 (LAB-018 · META-010) |
...
### 8-10. Zn-ion 은 조건이 둘 다릅니다 — 이 열을 논문과 나란히 놓지 마십시오

**하나. 학습률을 바꿨습니다.** 문서 값(CPMLP 5e-4 · CPTransformer 1e-3)으로는
훈련 손실이 **1.00 에서 내려오지 않습니다.** CPMLP/Zn-ion/2021 은 1에폭
1.04550 뒤 17에폭까지 1.0046 ~ 1.0081 사이를 맴돌다 조기 종료했습니다.
다른 세 도메인과 같은 **5e-05** 로 낮춘 값을 표에 실었습니다.

| 모델 | 문서 설정 | 5e-05 (표에 실은 값) |
|---|---:|---:|
| MLP | 1.215 | 1.215 — 바뀌지 않음 (원래 셸 값 5e-05) |
| CPMLP | 1.065 | 0.629 |
| CPTransformer | 0.642 | 0.509 |

배치와 `patience` 는 반증했습니다 — 배치 절반은 1.1999 로 더 나빴고
`patience` 증가는 0.9473 으로 기준과 같았습니다. 진단 스크립트는
`.build\batterylife\_diagnostics\` 이고 설명은 그 폴더의 `README.md` 입니다.
**정식 36개 스크립트는 문서 값 그대로 두었습니다** — 문서가 지정한 조건이
무엇이었는지가 남아 있어야 하기 때문입니다.

**둘. 모집단이 다릅니다.** 논문 Table 2 의 Zn-ion 은 **95셀**인데 코드 분할은
**100셀**입니다. v11 에서 이 서브셋 라벨이 26셀 늘었습니다. 5셀 차이 중
1셀은 설명됩니다 — 논문 부록 A.1 은 "수명 100 **이하** 제외" 라고 적는데
분할은 **100 미만**만 뺐고, 수명이 정확히 100 인 셀 하나가 train 에
남아 있습니다 (`findings` LAB-018). **나머지 4셀은 설명되지 않습니다.**

두 조건이 겹치므로 **Zn-ion 열의 숫자가 논문과 가깝다는 것을 재현의
근거로 쓰지 마십시오.**

### 8-5. Li-ion 은 843셀이 아니라 837셀입니다
...
> 843 - 6 = 837 이고 510+165+162 = 837 입니다.
>
> **837 은 논문 Table 2 의 Li-ion 셀 수와 같습니다** (2026-08-05 확인,
> `findings` META-010). 즉 6셀을 뺀 것은 논문에서 멀어지는 것이 아니라
> 맞는 쪽입니다 — 843 은 논문 어디에도 없는 수이고 갱신되지 않은 것은
> 코드 분할 파일 쪽입니다. 다만 **수가 맞는 것과 같은 셀을 뺀 것은 다른
> 말입니다.** 저자가 어떤 사유로 837 에 이르렀는지는 논문에 없습니다.
>
> 이름은 바꾸지 않았습니다 — 바꾸면 이미 만들어진 체크포인트 폴더 이름과
> 로그가 어긋납니다. `docs/reports/2026-08-04_unattended.md` §8.
